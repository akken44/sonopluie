...
DEBUG = False # True if you want to enable print() method
TIME_SCAN_BLE = 2 # By default, a scan last 2 seconds

# Define GPIO pins
PIN_BTN = 24
PIN_LED = 12
PIN_PWR_LED = 16 # This pin is used to send a high level to the led to be able to turn it on (it is connected to the 5V pin of the Led)
...
class App:
	...
	def initBtnLed(self):
		...
		GPIO.output(PIN_PWR_LED, True) # Send an high level to the 5V pin of the led

		# Start up the led
		self.led = Adafruit_NeoPixel(1, PIN_LED, 800000, 5, False, 100) # Adafruit_NeoPixel(LED_COUNT, LED_PIN, LED_FREQ_HZ, LED_DMA, LED_INVERT, LED_BRIGHTNESS)
		self.led.begin()
		self.led.active = True # This var is set False once the user shutdown the system
	...
	def checkBtn(self):
		...
		while True:
			...
			if durationPress >= LONG_PRESS:
				...
				subprocess.Popen(['shutdown', 'now']) # Use subprocess instead of sys.os() to avoid the waiting of the return of the cmd
				os._exit(1) # Use this instead of sys.exit() cause it will kill the thread only
	...
	def updateLed(self, color):
		if self.led.active:
			self.led.setPixelColor(0, color) # Setting the new color of the led
			self.led.show() # Update the led
	...
	def updateGPS(self):
		while True:
			...
			if self.mode:				
				...
				if _latitude is None or _longitude is None: # If GPS position has been found
					self.updateLed(COLOR_NOGPS)
					continue
				
				if int(self.latitude) != int(_latitude) or int(self.longitude) != int(_longitude): # Avoid some bugs when for example longitude is -1.2983687 and the next time is 1.2983687 
					continue
	...
	def updateBLE(self):
		...
		while True:
			beaconsScanned = self.ble.scan(TIME_SCAN_BLE, "b9407f30f5f8466eaff925556b57fe6d") # scan(timeout in seconds, UUID) - Here b9407f30f5f8466eaff925556b57fe6d is the default Estimote UUID
