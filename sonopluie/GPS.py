...
class GPS(object):
	...
	def __init__(self):
		self.ser = serial.Serial(port='/dev/ttyAMA0', baudrate = 9600) # Init the serial communication with the GPS
		self.latitude = None
		self.longitude = None
	...
	def updatePosition(self):
		...
		m = re.search(r"GPGGA,(\d*\.?\d*),(\d*\.?\d*),([A-Z]?),(\d*\.?\d*),([A-Z]?),", line) # Regex used to parse the trame
